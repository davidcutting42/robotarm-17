...
float homepos[] = {0, h + u, 0, 90}; 
float stack1[] = {-280, -213, 0, 0};
float stack2[] = {85, 68, 0, 0};
float stack3[] = {295, 277, 1, 0};
float stack4[] = {85, 487, 1, 0};
float stack5[] = {-124, 277, 1, 0};
float targetcenter[] = {85, 277, 1, 0};
...
int astepcount = 50*stpmode;
int bstepcount = 0*stpmode;
int cstepcount = 0*stpmode;

int astepdifference = 0;
int bstepdifference = 0;
int cstepdifference = 0;

int astepdirection = 0;
int bstepdirection = 0;
int cstepdirection = 0;

const int motadelay = 500;
const int motbdelay = 500;
const int motcdelay = 50;

int astepswitch = 0;
int bstepswitch = 0;
int cstepswitch = 0;
int astepswitchtimer = 0;
int bstepswitchtimer = 0;
int cstepswitchtimer = 0;
...
void loop() {
  switch (effectorpositionstatus) {
    case 0:
      ...
      if (/*digitalRead(startbutton) == 0 &&*/ astepdifference == 0 && bstepdifference == 0) {
        effectorpositionstatus = 1;
      }
      break;
    case 1:
      inversekinematics(stack1);
      if (astepdifference == 0 && bstepdifference == 0) {
...
void inversekinematics(float waypoint[]) {
  //Calculating the two different possibilities for the beta angle
  float beta1 = atan2(sqrt(1-((pow(waypoint[0], 2)+pow(waypoint[1], 2)-pow(h, 2)-pow(u, 2))/(2 * h * u))),(pow(waypoint[0], 2)+pow(waypoint[1], 2)-pow(h, 2)-pow(u, 2))/(2 * h * u));
  float beta2 = atan2(-sqrt(1-((pow(waypoint[0], 2)+pow(waypoint[1], 2)-pow(h, 2)-pow(u, 2))/(2 * h * u))),(pow(waypoint[0], 2)+pow(waypoint[1], 2)-pow(h, 2)-pow(u, 2))/(2 * h * u));
  ...
  float alpha1 = atan2(waypoint[1], waypoint[0])-atan2(k2a, k1a);
  float alpha2 = atan2(waypoint[1], waypoint[0])-atan2(k2b, k1b);
  ...
  if((trigresults[0] > 200 && trigresults[0] < 360) || (trigresults[2] > 154 && trigresults[2] < 334) || (trigresults[0] != trigresults[0]) || (trigresults[2] != trigresults[2])) {
    int checkone = false;
  }
  
  // Test alpha2 and beta2 to see if they are inside the allowable limits
  if((trigresults[1] > 200 && trigresults[1] < 360) || (trigresults[3] > 154 && trigresults[3] < 334) || (trigresults[1] != trigresults[1]) || (trigresults[3] != trigresults[3])) {
    int checktwo = false;
  }
  ...
  motormovesettings(angles, waypoint);
}
...
void motormovesettings(float angles[], float waypoint[]) {
  int stepperAtarget = map(angles[0], 0, 360, 0, 200*stpmode);
  int stepperBtarget = map(angles[1], 0, 360, 0, 200*stpmode);
  unsigned long stepperCtarget = waypoint[3] / 360 * 10000 * stpmode;
  ...
  if(stepperCtarget < waypoint[3]) {
    cstepdifference = waypoint[3] - stepperCtarget;
    digitalWrite(cdir, LOW); 
  }
  else if(stepperCtarget > waypoint[3]) {
    cstepdifference = stepperCtarget - waypoint[3];
